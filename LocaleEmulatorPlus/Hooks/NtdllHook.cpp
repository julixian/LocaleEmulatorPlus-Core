...
NoInline PVOID FASTCALL LoadSelfAsFirstDll(PVOID ReturnAddress)
{
    ...
    LOOP_ONCE
    {
        ...
        WriteProtectMemory(CurrentProcess, LEPPEB->LdrLoadDllAddress, LEPPEB->LdrLoadDllBackup, LEPPEB->LdrLoadDllBackupSize);
...
NTSTATUS LepGlobalData::InjectSelfToChildProcess(HANDLE Process, PCLIENT_ID Cid)
{
    NTSTATUS    Status;
    ...
    PVOID ooxxAddress = LEPPEB->LdrLoadDllAddress;
    BYTE ooxxBuffer[16];
    ...
    ooxxBuffer[0] = 0xFF;
    ooxxBuffer[1] = 0x25;
    *(PULONG)&ooxxBuffer[2] = 0;
    *(PVOID *)&ooxxBuffer[6] = ShadowLoadFirstDll;
#else
    LONG Offset;
    ...
    *(PULONG)&ooxxBuffer[1] = Offset;
    ...
    Status = WriteProtectMemory(Process, ooxxAddress, ooxxBuffer, LDR_LOAD_DLL_BACKUP_SIZE);
...
NTSTATUS LepGlobalData::HookNtdllRoutines(PVOID Ntdll)
{
    ...
    if (LdrInitNtContinue == nullptr)
        WriteLog(L"LdrInitNtContinue not found on x64");
#else
    if (LdrInitNtContinue == nullptr)
        return STATUS_NOT_SUPPORTED;
#endif
