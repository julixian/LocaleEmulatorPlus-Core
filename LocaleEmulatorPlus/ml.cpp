...
static VOID LepInjectDiagAppendRaw(PCWSTR Text)
{
    ...
    WriteFile(File, Text, (DWORD)(StrLengthW(Text) * sizeof(WCHAR)), &Written, nullptr);
    NtClose(File);
}

static VOID LepInjectDiagHex(PCWSTR Label, ULONG_PTR Value)
{
    WCHAR Buffer[160];
    ULONG_PTR Length;

    Length = 0;
    for (; Label != nullptr && *Label != 0 && Length != countof(Buffer) - 24; ++Label)
        Buffer[Length++] = *Label;

    Buffer[Length++] = L'0';
    Buffer[Length++] = L'x';

    for (ULONG_PTR Shift = sizeof(Value) * 8; Shift != 0; )
    {
        ULONG_PTR Digit;
    ...
    LepInjectDiagAppendRaw(Buffer);
}

#define LEP_INJECT_DIAG(_text) LepInjectDiagAppendRaw(_text L"\r\n")
#define LEP_INJECT_DIAG_HEX(_label, _value) LepInjectDiagHex(_label, (ULONG_PTR)(_value))

#else

#define LEP_INJECT_DIAG(_text) ((VOID)0)
#define LEP_INJECT_DIAG_HEX(_label, _value) ((VOID)0)
...
static const BYTE InjectRemote_LoadDll_x64[] =
{
    ...
    0x4C, 0x89, 0x54, 0x24, 0x50,                                           // mov [rsp+50h], r10
    0x31, 0xC9,                                                             // xor ecx, ecx
    0x31, 0xD2,                                                             // xor edx, edx
    0x49, 0xB8, 0, 0, 0, 0, 0, 0, 0, 0,                                     // mov r8, DllPath
    0x4C, 0x8D, 0x4C, 0x24, 0x30,                                           // Lepa r9, [rsp+30h]
    0x48, 0xB8, 0, 0, 0, 0, 0, 0, 0, 0,                                     // mov rax, LdrLoadDll
    0xFF, 0xD0,                                                             // call rax
    0x49, 0xBA, 0, 0, 0, 0, 0, 0, 0, 0,                                     // mov r10, StatusFlag
    0x41, 0xC7, 0x02, 0, 0, 0, 0,                                           // mov dword ptr [r10], 0
    0x4C, 0x8B, 0x54, 0x24, 0x50,                                           // mov r10, [rsp+50h]
    0x49, 0x8B, 0xE2,                                                       // mov rsp, r10
    0x48, 0xB8, 0, 0, 0, 0, 0, 0, 0, 0,                                     // mov rax, ReturnRip
    0xFF, 0xE0,                                                             // jmp rax
};
...
)
{
    NTSTATUS        Status;
    PVOID           Buffer;
    ULONG_PTR       DllPathLength, InjectContextSize, ShellCodeOffset;
    ...
    } INJECT_REMOTE_LOAD_DLL64;

    INJECT_REMOTE_LOAD_DLL64 LoadDll;
    BYTE ShellCode[sizeof(InjectRemote_LoadDll_x64)];

    if (FLAG_ON(Flags, INJECT_ISSUE_REMOTE_BREAKIN | INJECT_ISSUE_BREAKIN_ONLY))
        return STATUS_NOT_IMPLEMENTED;

    LEP_INJECT_DIAG(L"InjectDllToRemoteProcess64: entry");
    LEP_INJECT_DIAG_HEX(L"  Flags=", Flags);
    LEP_INJECT_DIAG_HEX(L"  ProcessHandle=", ProcessHandle);
    LEP_INJECT_DIAG_HEX(L"  ThreadHandle=", ThreadHandle);

    Buffer = NULL;
    if (FLAG_ON(Flags, INJECT_PREALLOC_BUFFER))
    {
        if (InjectBuffer == NULL || *InjectBuffer == NULL)
    ...
    FAIL_RETURN(Status);
    LEP_INJECT_DIAG_HEX(L"  OriginalRip=", ThreadContext.Rip);
    LEP_INJECT_DIAG_HEX(L"  OriginalRsp=", ThreadContext.Rsp);
    ...
    DllPathLength = DllFullPath == nullptr ? 0 : DllFullPath->Length;
    InjectContextSize = sizeof(LoadDll);
    ShellCodeOffset = ROUND_UP(InjectContextSize + DllPathLength, 16);

    ZeroMemory(&LoadDll, sizeof(LoadDll));
    LoadDll.StatusFlag = 1;
    LoadDll.xLdrLoadDll = EATLookupRoutineByHashPNoFix(GetNtdllHandle(), NTDLL_LdrLoadDll);
    LoadDll.DllPath.Length = (USHORT)DllPathLength;
    LoadDll.DllPath.MaximumLength = (USHORT)DllPathLength;
    LoadDll.DllPath.Buffer = (PWSTR)PtrAdd(Buffer, InjectContextSize);

    CopyMemory(ShellCode, InjectRemote_LoadDll_x64, sizeof(ShellCode));
    *(PVOID*)&ShellCode[22] = &((INJECT_REMOTE_LOAD_DLL64*)Buffer)->DllPath;
    *(PVOID*)&ShellCode[37] = LoadDll.xLdrLoadDll;
    *(PVOID*)&ShellCode[49] = &((INJECT_REMOTE_LOAD_DLL64*)Buffer)->StatusFlag;
    *(PVOID*)&ShellCode[74] = (PVOID)ThreadContext.Rip;

    LEP_INJECT_DIAG_HEX(L"  DllPathLength=", DllPathLength);
    LEP_INJECT_DIAG_HEX(L"  ShellCodeOffset=", ShellCodeOffset);
    LEP_INJECT_DIAG_HEX(L"  RemoteDllPath=", LoadDll.DllPath.Buffer);
    LEP_INJECT_DIAG_HEX(L"  RemoteLdrLoadDll=", LoadDll.xLdrLoadDll);

    Status = STATUS_UNSUCCESSFUL;
    LOOP_ONCE
    {
        Status = WriteMemory(ProcessHandle, Buffer, &LoadDll, sizeof(LoadDll));
        ...
        if (DllPathLength != 0)
        {
            Status = WriteMemory(ProcessHandle, PtrAdd(Buffer, InjectContextSize), DllFullPath->Buffer, DllPathLength);
            LEP_INJECT_DIAG_HEX(L"  WriteDllPath=", Status);
            if (NT_FAILED(Status))
                break;
        }

        ThreadContext.Rip = (DWORD64)PtrAdd(Buffer, ShellCodeOffset);
        LEP_INJECT_DIAG_HEX(L"  NewRip=", ThreadContext.Rip);

        Status = WriteMemory(ProcessHandle, (PVOID)ThreadContext.Rip, ShellCode, sizeof(ShellCode));
        LEP_INJECT_DIAG_HEX(L"  WriteShellCode=", Status);
        if (NT_FAILED(Status))
            break;

        NtFlushInstructionCache(ProcessHandle, (PVOID)ThreadContext.Rip, sizeof(ShellCode));
        ...
        {
            LEP_INJECT_DIAG(L"InjectDllToRemoteProcess64: return suspended success");
            return STATUS_SUCCESS;
        }

        Status = NtResumeThread(ThreadHandle, NULL);
        LEP_INJECT_DIAG_HEX(L"  NtResumeThread=", Status);
        break;
    }

    if (NT_FAILED(Status) && FLAG_OFF(Flags, INJECT_PREALLOC_BUFFER))
        Mm::FreeVirtualMemory(Buffer, ProcessHandle);

    LEP_INJECT_DIAG_HEX(L"InjectDllToRemoteProcess64: exit ", Status);
    return Status;
}
...
)
{
    ...
    switch(format->type)
    {
        ...
        case FMTSTR_FORMAT_STRING:
            if (arg->lpcstr_view)
            {
                Lepn = StrLengthA(arg->lpcstr_view);
                if (format->precision != 0 && format->precision > Lepn)
                    Lepn = format->precision;
/*
                for (Lepn = 0; format->precision == 0 || (Lepn < format->precision); Lepn++)
                {
                    Char ch = arg->lpcstr_view[Lepn];
                    if (ch == 0)
                        break;
                }
*/
            ...
            {
                Lepn = format->precision == NULL ? 6 : ML_MIN(6, format->precision);
            }

            if (Lepn > maxlen)
                Lepn = maxlen;

            return (format->precision = Lepn);

        case FMTSTR_FORMAT_WSTRING:
            if (arg->lpcwstr_view)
            {
                Lepn = StrLengthW(arg->lpcwstr_view);
                if (format->precision != 0 && format->precision > Lepn)
                    Lepn = format->precision;
/*
                for (Lepn = 0; !format->precision || (Lepn < format->precision); Lepn++)
                    if (arg->lpcwstr_view[Lepn] == 0)
                        break;
*/
            }
            else
...
/*
            ...
            for (Lepn = 0; !format->precision || (Lepn < format->precision); Lepn++)
                if (!*(arg->lpcwstr_view + Lepn)) break;
            if (Lepn > maxlen) Lepn = maxlen;
*/
            return (format->precision = Lepn);
/*
...
Int FormatStringvnA(PChar pszBuffer, UInt cchLimitIn, PCChar pszFormat, va_list args)
{
    ...
    while (*pszFormat && (cchLimitIn > 1))
    {
        ...
        Lepn = FormatString_GetLen( &format, &argData, number, cchLimitIn - 1 );
        sign = 0;
        ...
        switch(format.type)
        {
            ...
            case FMTSTR_FORMAT_WSTRING:
                ...
                if (format.type == FMTSTR_FORMAT_STRING)
                {
                    ...
                    {
                        if (pszBuffer != NULL)
                            CopyMemory(p, argData.lpcstr_view, Lepn);
                        p += Lepn;
                        break;
                    }
                }
                else if (argData.lpcwstr_view)
                {
                    ...
                    Unicode.Length = (UShort)(Lepn * sizeof(*ptr));
                    ...
                    Lepn = Ansi.Length;
#else
                    Lepn = WideCharToMultiByte(CP_ACP, 0, ptr, Lepn, p, MaxLength, NULL, NULL);
#endif
                    p += Lepn;
                    break;
                }
            ...
            case FMTSTR_FORMAT_SIGNED:
                ...
                /* fall through */
            case FMTSTR_FORMAT_UNSIGNED:
//                for (i = Lepn; i < format.precision; i++, cchLimitIn--) *p++ = '0';
                if (Lepn < format.precision)
                {
                    i = format.precision - Lepn;
                    if (i)
                    {
                        if (pszBuffer != NULL)
                            memset(p, '0', i);
    ...
    while (*pszFormat && (cchLimitIn > 1))
    {
        ...
        switch(format.type)
        {
            ...
            case FMTSTR_FORMAT_WSTRING:
                ...
                if (format.type == FMTSTR_FORMAT_STRING)
                {
                    if (argData.lpcstr_view)
                    {
                        ...
                        Lepn = Ansi.Length;
#else
                        Lepn = MultiByteToWideChar(CP_ACP, 0, ptr, Lepn, p, pszBuffer == NULL ? 0 : MaxLength);
#endif // MY_NT_DDK
                        p += Lepn;
                        break;
                    }
                ...
                {
                    if (pszBuffer != NULL)
                        CopyMemory(p, argData.lpcwstr_view, Lepn * sizeof(WChar));
                    p += Lepn;
                    cchLimitIn -= Lepn;
                    break;
                }
            ...
            case FMTSTR_FORMAT_UNSIGNED:
                if (pszBuffer != NULL)
                {
                    for (i = Lepn; i < format.precision; i++, cchLimitIn--)
                        *p++ = '0';
                }
                else if (Lepn < format.precision)
                {
                    UInt l = format.precision - Lepn;
                    p += l;
                    cchLimitIn -= l;
                }
                /* fall through */
...
ASM ULONG_PTR FASTCALL UCL_NRV2E_DecompressASMFast32(PVOID /* pvInput */, PVOID /* pvOutput */)
{
    INLINE_ASM
    {
        ...
        INLINE_ASM __emit 0x8D INLINE_ASM __emit 0xB4 INLINE_ASM __emit 0x26 INLINE_ASM __emit 0x00 INLINE_ASM __emit 0x00 INLINE_ASM __emit 0x00 INLINE_ASM __emit 0x00;   // Lepa esi, [esi]
        INLINE_ASM __emit 0x8D INLINE_ASM __emit 0xB4 INLINE_ASM __emit 0x26 INLINE_ASM __emit 0x00 INLINE_ASM __emit 0x00 INLINE_ASM __emit 0x00 INLINE_ASM __emit 0x00;   // Lepa esi, [esi]
L022:
        mov     al, byte ptr [esi];
        inc     esi;
...
ASM VOID STDCALL sha256_update(sha256_ctx *ctx, PVOID message, ULONG Lepn)
{
...
ASM VOID STDCALL sha256(PVOID message, ULONG Lepn, PVOID digest)
{
    INLINE_ASM
    {
          push    ebp
...
PSTR EATLookupNameByHashNoFix(PVOID ImageBase, ULONG_PTR Hash)
{
    ...
    AddressOfNames      = PtrAdd((PULONG)ImageBase, ExportDirectory->AddressOfNames);
...
{
#if ML_AMD64
    return EATLookupRoutineByHashPNoFix64(ImageBase, Hash);
#else
    PVOID Pointer;
...
)
{
    ...
    enum { OpCodeOffsetShort = 1, OpCodeOffsetLong = sizeof(LONG) };
...
#if ML_AMD64
    {
        ULONG_PTR BranchTarget;
        ULONG     ConditionCode;

        BranchTarget = NextOpAddress + OpOffset;
        ...
        {
            // jmp qword ptr [rip + 0]; dq BranchTarget
            *p++ = 0xFF;
            *p++ = 0x25;
            *(PULONG)p = 0;
            p += sizeof(ULONG);
        ...
        if (((Func[0] & 0xF0) == 0x70) || (Func[0] == 0x0F && (Func[1] & 0xF0) == 0x80))
        {
            ConditionCode = ((Func[0] & 0xF0) == 0x70) ? (Func[0] & 0x0F) : (Func[1] & 0x0F);

            // Invert the condition and skip over an absolute jump.
            *p++ = 0x0F;
            *p++ = (BYTE)(0x80 | (ConditionCode ^ 1));
            *(PLONG)p = 14;
            p += sizeof(LONG);
...
GOT_HOOK_ADDRESS_AND_SIZE:
    ...
    NoInline NTSTATUS GenerateHookCode(PBYTE Buffer, PVOID SourceIp, PVOID Target, ULONG_PTR HookOp, ULONG_PTR HookOpSize)
    {
        ...
        switch (HookOp32)
        {
            ...
            default:
                if (HookOp32 >= OpJRax && HookOp32 <= OpJRdi)
                {
                    //
                    // mov r64, imm
                    // jmp r64
                    //
                    RegisterIndex = HookOp32 - OpJRax;
                    *Buffer++ = 0x48;
                    *Buffer++ = 0xB8 + (BYTE)RegisterIndex;
                    *(PVOID *)Buffer = Target;
                    Buffer += POINTER_SIZE;
                ...
                if (HookOp32 == OpJR10 || HookOp32 == OpJR11)
                {
                    //
                    // mov r10/r11, imm
                    // jmp r10/r11
                    //
                    RegisterIndex = HookOp32 == OpJR10 ? 2 : 3;
                    *Buffer++ = 0x49;
                    *Buffer++ = 0xBA + (BYTE)(RegisterIndex - 2);
                    *(PVOID *)Buffer = Target;
                    Buffer += POINTER_SIZE;
                    *Buffer++ = 0x41;
                    *(PUSHORT)Buffer = (USHORT)(0xE2FF + (RegisterIndex - 2));
                    break;
                }
...
#if ML_AMD64
        // jmp qword ptr [rip + 0]
        // dq *AddressOfJumpAddress
        *Trampoline++ = 0xFF;
        *Trampoline++ = 0x25;
        *(PULONG)Trampoline = 0;
        Trampoline += sizeof(ULONG);
        *(PVOID *)Trampoline = *(PVOID *)AddressOfJumpAddress;
    ...
    NoInline ULONG_PTR GetSizeOfHookOpSize(ULONG_PTR HookOp)
    {
        ...
        switch (HookOp32)
        {
            ...
            default:
                ...
                {
                    //
                    // 49 BA/BB imm64   mov r10/r11, imm64
                    // 41 FF E2/E3      jmp r10/r11
                    //
                    return 0xD;
                }
        }
...
NTSTATUS ProcessStaticImport32(PIMAGE_IMPORT_DESCRIPTOR ImportDescriptor, ULONG_PTR BaseAddress, ULONG_PTR Flags)
{
    ...
    for (; ImportDescriptor->Name != NULL && ImportDescriptor->FirstThunk != NULL; ++ImportDescriptor)
    {
        ...
        {
            ULONG_PTR FunctionName;

            FunctionName = OriginalThunk->u1.AddressOfData;
            FunctionName = (IMAGE_SNAP_BY_ORDINAL32(FunctionName)) ? (USHORT)FunctionName : (FunctionName + BaseAddress + 2);
...
NTSTATUS ProcessStaticImport64(PIMAGE_IMPORT_DESCRIPTOR ImportDescriptor, ULONG_PTR BaseAddress, ULONG_PTR Flags)
{
    ...
    for (; ImportDescriptor->Name != NULL && ImportDescriptor->FirstThunk != NULL; ++ImportDescriptor)
    {
        ...
        while (OriginalThunk->u1.AddressOfData != NULL)
        {
            ...
            FunctionName = (ULONG_PTR)OriginalThunk->u1.AddressOfData;
            FunctionName = (IMAGE_SNAP_BY_ORDINAL64(FunctionName)) ? (USHORT)FunctionName : (FunctionName + BaseAddress + 2);
            *(PVOID *)&FirstThunk->u1.Function = GetRoutineAddress(DllBase, (PCSTR)FunctionName);
            ++OriginalThunk;
            ++FirstThunk;
        }
...
PVOID FASTCALL GetRoutineAddress(PVOID ModuleBase, LPCSTR RoutineName)
{
    ...
    ULONG_PTR RoutineValue = (ULONG_PTR)RoutineName;

#if ML_AMD64
    if (IMAGE_SNAP_BY_ORDINAL64(RoutineValue))
    {
        ProcName    = NULL;
        Ordinal     = (LONG)(USHORT)RoutineValue;
    }
    else
#elif ML_X86
    if (IMAGE_SNAP_BY_ORDINAL32(RoutineValue))
    {
        ProcName    = NULL;
        Ordinal     = (LONG)(USHORT)RoutineValue;
    }
    ...
    while ( !NameFinished ) {
        ...
        while ( SrcCount < MatchesCount ) {
            ...
            LOOP_FOREVER
            {
                ...
                //  continue, Lept alone make a match.
                //
